...
export default function AdminConsultationsPage() {
  ...
  const fetch = async () => {
    setIsLoading(true);
    try { const res = await apiGet<any>('/consultations'); setConsultations(res.data.consultations); }
  ...
  const openDetail = (c: Consultation) => {
    setSelected(c); setStatus(c.status); setAdminNotes(c.adminNotes || '');
  ...
  const handleUpdate = async () => {
    ...
    setSaving(true);
    try {
      await apiPut(`/consultations/${selected.id}`, { status, adminNotes, scheduledAt: scheduledAt || undefined });
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        <div className="lg:col-span-2 bg-white rounded-2xl border border-gray-100 overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              {isLoading ? Array.from({ length: 8 }).map((_, i) => <TableRowSkeleton key={i} cols={5} />) :
               consultations.length === 0 ? <tr><td colSpan={5}><EmptyState title="No consultations yet" /></td></tr> :
               consultations.map((c) => (
                <tr key={c.id} className={`border-b border-gray-50 hover:bg-gray-50/50 cursor-pointer transition-colors ${selected?.id === c.id ? 'bg-brand-50/50' : ''}`}
                  onClick={() => openDetail(c)}>
                  <td className="px-4 py-3">
                    <p className="font-medium text-gray-900">{c.customerName}</p>
                    <p className="text-xs text-gray-400">{c.customerEmail}</p>
                  </td>
                  <td className="px-4 py-3 text-gray-700 max-w-[200px] line-clamp-1 text-xs">{c.subject}</td>
                  <td className="px-4 py-3 text-gray-400 text-xs">
                    {c.preferredDate ? `${c.preferredDate}${c.preferredTime ? ' ' + c.preferredTime : ''}` : 'Flexible'}
                  </td>
                  <td className="px-4 py-3">
        ...
        {selected ? (
          <div className="bg-white rounded-2xl border border-gray-100 p-5 space-y-4">
            ...
            <div className="text-sm space-y-1">
              <p><span className="text-gray-500">Name:</span> <span className="font-medium">{selected.customerName}</span></p>
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 bg-gray-50 rounded-lg p-3 text-xs">{selected.message}</p>
            </div>
            <div>
