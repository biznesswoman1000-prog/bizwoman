...
import { formatPrice, formatDateTime, ORDER_STATUS_COLORS, ORDER_STATUS_LABELS, PAYMENT_STATUS_COLORS, getProductImage } from '@/lib/utils';
...
export default function AdminOrderDetailPage() {
  ...
  const loadOrder = () => {
    setIsLoading(true);
    apiGet<any>(`/orders/${id}`).then((res) => {
      setOrder(res.data.order);
      setNewStatus(res.data.order.status);
      setTrackingNumber(res.data.order.trackingNumber || '');
    }).catch(() => setError('Order not found')).finally(() => setIsLoading(false));
  };
  ...
  const updateStatus = async () => {
    ...
    setUpdating(true);
    try {
      await apiPut(`/orders/${id}/status`, { status: newStatus, notes: statusNotes || undefined, trackingNumber: trackingNumber || undefined });
      toast('Order updated', 'success'); setStatusNotes(''); loadOrder();
    } catch (err) { toast(getApiError(err), 'error'); }
    finally { setUpdating(false); }
  };

  if (isLoading) return <PageLoader />;
  if (error || !order) return <div className="p-6"><ErrorState message={error || 'Not found'} /></div>;

  return (
    <div className="p-6 max-w-5xl space-y-5">
      <div className="flex items-center gap-4">
        <Link href="/admin/orders" className="p-2 rounded-lg border border-gray-200 text-gray-500 hover:text-gray-700 transition-colors">
        ...
        <div className="flex gap-2">
          ...
          <span className={`px-3 py-1 rounded-full text-sm font-medium ${PAYMENT_STATUS_COLORS[order.paymentStatus]}`}>{order.paymentStatus}</span>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-5">
            ...
            <div className="space-y-3">
              {order.items.map((item) => (
                <div key={item.id} className="flex gap-3 items-center">
                  <img src={getProductImage(item.product.images)} alt={item.product.name} className="w-12 h-12 rounded-lg object-cover border border-gray-100 shrink-0" />
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-gray-900 text-sm line-clamp-1">{item.product.name}</p>
                    <p className="text-xs text-gray-400 font-mono">SKU: {item.product.sku} · Qty: {item.quantity}</p>
                  </div>
            ...
            <div className="mt-4 pt-4 border-t border-gray-100 space-y-2 text-sm">
              ...
              <div className="flex justify-between font-bold text-base border-t border-gray-100 pt-2"><span>Total</span><span className="text-brand-700">{formatPrice(order.total)}</span></div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-5">
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input value={trackingNumber} onChange={(e) => setTrackingNumber(e.target.value)} placeholder="e.g. GIG12345"
              ...
              <div className="space-y-3">
                {order.statusHistory.map((h) => (
                  <div key={h.id} className="flex gap-3 text-sm">
                    ...
                    <div>
                      <p className="font-medium text-gray-900">{ORDER_STATUS_LABELS[h.status] || h.status}</p>
        ...
        <div className="space-y-5">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-5">
            ...
            <div className="text-sm space-y-1">
              <p className="text-gray-600">Method: <span className="font-medium text-gray-900">{order.paymentMethod}</span></p>
              {order.paymentReference && <p className="text-gray-600">Ref: <span className="font-mono text-xs">{order.paymentReference}</span></p>}
            </div>
          </div>
        </div>
      </div>
