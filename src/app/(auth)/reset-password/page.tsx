...
export default function ResetPasswordPage() {
  ...
  const onSubmit = async (data: ResetPasswordForm) => {
    if (!token) { toast('Invalid or expired reset link', 'error'); return; }
    setIsLoading(true);
    try {
      ...
      <div className="bg-white rounded-2xl shadow-card border border-gray-100 p-8">
        {done ? (
          <div className="text-center">
            ...
            <h2 className="font-display text-2xl font-bold text-gray-900">Password updated!</h2>
            <p className="mt-2 text-sm text-gray-500">Your password has been changed successfully.</p>
            <Link href="/login" className="mt-6 inline-flex px-6 py-2.5 bg-brand-600 text-white rounded-xl text-sm font-medium hover:bg-brand-700 transition-colors">
              Sign In
            </Link>
          </div>
        ) : (
          <>
            <div className="mb-8">
              ...
              <p className="mt-2 text-sm text-gray-500">Choose a strong new password for your account.</p>
            </div>
