...
export default function CartPage() {
  const { items, subtotal, itemCount, updateQuantity, removeFromCart, clearCart, isLoading } = useCart();
    ...
    return (
      <div className="container py-16">
        <EmptyState
          ...
          action={
            <Link href="/products" className="px-6 py-3 bg-brand-600 text-white rounded-xl font-semibold hover:bg-brand-700 transition-colors">
              Start Shopping
            </Link>
          }
        />
    ...
    <div className="container py-10 lg:py-14">
      ...
      <div className="grid lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-2 space-y-4">
          {items.map((item) => (
            <div key={item.id} className="flex gap-5 bg-white rounded-2xl border border-gray-100 p-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="mt-1 text-lg font-bold text-brand-700">{formatPrice(item.price)}</p>
                <div className="flex items-center justify-between mt-3">
                  <div className="flex items-center gap-1 border border-gray-200 rounded-xl px-1">
                    ...
                    <button onClick={() => updateQuantity(item.id, item.quantity + 1)}
                      disabled={item.quantity >= item.product.stockQuantity || isLoading}
                      className="p-2 text-gray-500 hover:text-gray-900 disabled:opacity-40 transition-colors">
