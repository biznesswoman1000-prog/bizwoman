...
export default function CategoriesPage() {
  ...
  useEffect(() => {
    apiGet<any>('/categories')
      .then((res) => setCategories(res.data.categories.filter((c: Category) => !c.parentId)))
      .catch(() => {})
  ...
  return (
    <div className="container py-10">
      <h1 className="font-display text-3xl font-bold text-gray-900 mb-2">Shop by Category</h1>
      <p className="text-gray-500 mb-8">Browse our curated collection of business essentials.</p>
            ...
            <Link key={cat.id} href={`/categories/${cat.slug}`}
              ...
              <div className="aspect-square bg-gradient-to-br from-brand-50 to-brand-100 flex items-center justify-center overflow-hidden">
                {cat.image ? (
                  ...
                  <span className="text-5xl text-brand-200 font-bold">{cat.name.charAt(0)}</span>
                )}
              </div>
              <div className="p-4">
                <h2 className="font-semibold text-gray-900 group-hover:text-brand-700 transition-colors">{cat.name}</h2>
