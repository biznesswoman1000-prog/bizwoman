...
interface HeroSlide {
  id?: string; // ✅ Add unique ID for each slide
  heading?: string;
  text?: string;
  buttonText?: string;
  buttonUrl?: string;
...
interface HeroBanner {
  id?: string; // ✅ Add unique ID for each banner
  heading?: string;
  text?: string;
  buttonText?: string;
  buttonUrl?: string;
...
export default function AdminHomeSettingsTab() {
  ...
  const loadSettings = async () => {
    try {
      ...
      // ✅ Convert old format to new format with unique IDs
      const slides = (settings.heroSlides || []).map((slide: any) => ({
        ...slide,
        id: slide.id || generateId(), // Add ID if missing
        media:
          slide.media ||
      ...
      const banners = (settings.heroBanners || []).map((banner: any) => ({
        ...banner,
        id: banner.id || generateId(), // Add ID if missing
        media:
          banner.media ||
  ...
  const saveHeroSlides = async () => {
    setSaving(true);
    try {
      // Convert media array to single image URL for backward compatibility
  ...
  const addSlide = () => {
    setHeroSlides([
      ...heroSlides,
      { id: generateId(), media: [], animation: "fade" }, // ✅ Add unique ID
    ]);
  };
  ...
  const addBanner = () => {
    ...
    setHeroBanners([
      ...heroBanners,
      { id: generateId(), media: [] }, // ✅ Add unique ID
    ]);
  };
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-6 space-y-6">
        ...
        <div className="space-y-4">
          {heroSlides.map((slide, index) => (
            <div
              key={slide.id || index} // ✅ Use unique ID as key
            ...
            >
              ...
              {/* ✅ Each uploader now has a unique key based on slide ID */}
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-6 space-y-6">
        ...
        <div className="space-y-4">
          {heroBanners.map((banner, index) => (
            <div
              key={banner.id || index} // ✅ Use unique ID as key
              className="border border-gray-200 rounded-xl p-4 space-y-3"
            >
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm font-semibold text-gray-700">
              ...
              {/* ✅ Each uploader now has a unique key based on banner ID */}
              <DragDropMediaUploader
      ...
      <div className="lg:col-span-2 relative h-64 rounded-lg overflow-hidden bg-gray-900">
        ...
        <div className="absolute inset-0 flex items-center p-6">
          <div>
            ...
            {slide.buttonText && (
              <button className="px-4 py-2 bg-gold-500 text-white rounded-lg text-sm">
                {slide.buttonText}
              </button>
            )}
          </div>
        ...
        {banners.slice(0, 2).map((banner, i) => (
          ...
          >
            ...
            <div className="absolute bottom-0 left-0 right-0 p-3">
              {banner.heading && (
                <p className="text-white font-bold text-sm">{banner.heading}</p>
              )}
              {banner.text && (
